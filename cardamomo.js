 ...
 */
export async function drawXAxisFromArray(context, xValues, xPos = 50, yPos = 50, color = 'black', labelSpace = 20, canvasPadding = context.canvas.width * 0.02) {
  ...
  const xCanvasValues = xValues.map(value => mapValue(value, canvasPadding, canvasWidth, xValues));
  ...
  for (let i = 0; i < xCanvasValues.length; i++) {
      const tickX = xPos + canvasPadding + (xCanvasValues[i] - startX);
      ...
      drawText(context, xValues[i], tickX - 5, yPos + labelSpace, 10, "black", 0);
  }
}
 ...
 */
export async function drawYAxisFromArray(context, yValues, xPos = 50, yPos = 50, color = 'black',labelSpace = -10, canvasPadding = context.canvas.width * 0.02) {

  // Ajustar el ancho del canvas teniendo en cuenta x y el padding
  const canvasHeight = context.canvas.height - 2 * canvasPadding;

  // Mapear los valores de datos a posiciones en el canvas
  const yCanvasValues = yValues.map(value => mapValue(value, canvasPadding, canvasHeight, yValues));
  ...
  for (let i = 0; i < yCanvasValues.length; i++) {
      const tickY = yPos + canvasPadding +  (yCanvasValues[i] - starty);
      ...
      // Dibujar el número debajo de la marca
      drawText(context,yValues[i],xPos-labelSpace,tickY, 10,"black",0)
  }
}
 ...
 * @param {number} canvasPadding - padding del canvas
 */
export async function drawDotPlot(canvas, context, csvFilePath, xColumnName, yColumnName, infoColumNames, color="black", canvasPadding = context.canvas.width * 0.02) {
  ...
  drawPoints(context,dataWithCanvasValues,color);
 ...
 * @param {number} canvasPadding - padding del canvas
 */
export async function drawLinePlot(canvas, context, csvFilePath, xColumnName, yColumnName, infoColumNames, color, canvasPadding = context.canvas.width * 0.02) {
  ...
  context.arc(dataWithCanvasValues[0].xCanvas, dataWithCanvasValues[0].yCanvas, pointRadius, 0, 2 * Math.PI);
  ...
  for (let i = 1; i < dataWithCanvasValues.length; i++) {
    ...
    context.arc(dataWithCanvasValues[i].xCanvas, dataWithCanvasValues[i].yCanvas, pointRadius, 0, 2 * Math.PI);
 ...
 * @param {number} canvasPadding - padding del canvas
 */
export async function drawBubblePlot(canvas, context, csvFilePath, xColumnName, yColumnName, sizeColumnName, minSize, maxSize, infoColumNames, color="black", canvasPadding = context.canvas.width * 0.02) {
 ...
 * @param {number} canvasPadding - padding del canvas
 */
export async function drawMapDotPlot(canvas, context, mapDataFile, csvFilePath, xColumnName, yColumnName, infoColumNames, color="black", canvasPadding = context.canvas.width * 0.02) {
...
export function drawPoints(context,dataWithCanvasValues,color){

  context.strokeStyle = color;
  context.fillStyle = color;

  const pointRadius = 5; // Radio de los puntos
